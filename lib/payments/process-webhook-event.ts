...
/**
 * Shared body of every payment webhook route, once a provider has already
 * verified the request's signature. Each provider has its own route (its own
 * URL, configured on the provider's side, and its own signature scheme) —
 * this is the part that's identical after that: idempotency guard, order
 * lookup, and the credit/refund RPCs.
 *
 * Order of operations matters:
 *   1. the caller verifies the signature before this function is ever called
